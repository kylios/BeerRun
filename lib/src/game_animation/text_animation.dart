...
class TextAnimation extends GameAnimation {
    ...
    TextAnimation(this._text, int x, int y, int duration) : super(x, y) {
        this._endTime = new DateTime.now().millisecondsSinceEpoch + duration *
                1000;
    }
    ...
    _TextAnimationFrame _getNext() {
        _TextAnimationFrame frame = new _TextAnimationFrame(0, -this._cur,
                this._text);
        if (this._cur < 16) {
            this._cur++;
        }
    ...
    bool get isDone => (new DateTime.now().millisecondsSinceEpoch >=
            this._endTime);
}

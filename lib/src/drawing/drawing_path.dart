      ...
      if (first) {
        ...
        if (c is GamePoint && c.x != -1 && c.y != -1) {
          ...
          if (lastStartAngle != -1 && lastEndAngle != -1) {
            ...
            int distance =
                sqrt((c.x - lastX) * (c.x - lastX) + (c.y - lastY) * (c.y - lastY))
                .toInt()
                .abs();
            int radius = distance ~/ 2;
            window.console.log("distance=$distance, radius=$radius");

            window.console.log("arc from ($lastX, $lastY) to (${c.x},${c.y}) from $lastStartAngle to $lastEndAngle, counterClockWise? ${lastCounterClockwise}");
            this._c.arc(lastX + radius, lastY, radius, lastStartAngle, lastEndAngle,
                lastCounterClockwise);
          } else {

...
class NPC extends GameObject implements GameEventListener {
    ...
    NPC(Level l, Direction d, int x, int y, this._name) : super(d, x, y) {
        ...
        SpriteSheet sprites = new SpriteSheet("assets/sprites/bum.png", 64, 64);
    ...
    void update() {
        if (!this.isRemoved) {
            ...
            if (this._damaged) {
                ...
                if (this._damagedUntil <= new DateTime.now(
    ...
    void listen(GameEvent e) {
        if (e.type == GameEvent.TAKE_HIT_EVENT) {
            if (!this._damaged) {
                ...
                this._damaged = true;
                this._damagedUntil = new DateTime.now().millisecondsSinceEpoch +
                        2000;
            }
            if (this._health <= 0) {

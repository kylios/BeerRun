...
class Player extends GameObject implements GameEventListener {
    ...
    int _nextBuzzDecreaseTS = new DateTime.now().millisecondsSinceEpoch ~/ 1000
            + 35;
    ...
    Player(this._mgr, this._stats) : super(DIR_DOWN, 0, 0) {
        ...
        SpriteSheet sprites = new SpriteSheet(
                "assets/sprites/player/player.png", 64, 64);
    ...
    void _blink(DateTime now) {
        if (this._damagedUntil <= now.millisecondsSinceEpoch) {
            ...
            if (this._damageInterval > -1 && this._damageInterval <=
                ...
                this._damageInterval = now.millisecondsSinceEpoch + 17;
            }
        }
    }
    ...
    void update() {
        ...
        if (!this.isRemoved) {
            ...
            if (this._beerStolenUntil <= new DateTime.now(
                    ).millisecondsSinceEpoch) {
                this._beerStolenUntil = 0;
            }

            if (this._buzzDecreaseTime > 0 && this._buzzDecreaseTime <=
                    now.millisecondsSinceEpoch) {
                this._buzz--;
                this._buzzDecreaseTime = now.millisecondsSinceEpoch +
                        Player.BUZZ_TIME;
                if (this._buzz <= 3 && !this._boredNotify) {
                    this.level.addAnimation(new TextAnimation("*YAWN*", this.x,
                            this.y, 3));

                    GameNotification n = new GameNotification(
                            "Your buzz is wearing off!  Drink a beer before things get too boring.<br />Use [space] to crack one open.",
                            imgUrl: "assets/ui/keyboard/Keyboard_White_Space.png");
    ...
    void listen(GameEvent e) {
        ...
        if (e.type == GameEvent.TAKE_HIT_EVENT) {
            if (!this._damaged) {
                ...
                this.level.addAnimation(new TextAnimation("OUCH!", this.x,
                        this.y, 2));
            ...
            if (this._health <= 0) {
                this.level.addAnimation(new Explosion.createAt(this.x, this.y,
                        this.tileWidth, this.tileHeight));
                this.remove();
            }
        } else if (e.type == GameEvent.BEER_STOLEN_EVENT) {
            if (this._beerStolenUntil == 0 && this._beers > 0) {
                this._beerStolenUntil = new DateTime.now(
                        ).millisecondsSinceEpoch + 1000;
                this._beers -= e.value;
                this.level.addAnimation(new TextAnimation("-1 BEER!", this.x,
                        this.y, 2));
                ...
                if (!this._wasBeerStolen) {
                    GameNotification n = new GameNotification(
                            "Ohhh, the bum stole a beer!  One less for you!");
                    this.broadcast(n, [this._mgr]);
            ...
            if (this._beers < e.value) {
                ...
                this.level.addAnimation(new TextAnimation("+${diff} BEERS!",
                        this.x, this.y, 2));
            }
            this._beers = e.value;
            ...
            if (this._beenToStore && this._beers > 0) {
                ...
                this.level.addAnimation(new TextAnimation("FUCK YEAH!", this.x,
                        this.y, 2));
    ...
    void drinkBeer() {
        ...
        this.level.addAnimation(new TextAnimation("+${BUZZ_PER_BEER} BUZZ",
                this.x, this.y - 16, 3));
        this.level.addAnimation(new TextAnimation("-1 BEER", this.x, this.y + 8,
                3));

        if (this._buzz >= 8 && !this._drunkNotify) {
            GameNotification n = new GameNotification(
                    "Be careful, don't get too drunk!");
            this.broadcast(n, [this._mgr]);
    ...
    void spawnBullet() {
        ...
        this.broadcast(new CreateBulletEvent(this.dir, this, x, y), [this.level]
                );
    }
    ...
    Sprite getMoveSprite() {
        if (this._damaged && this._damageInterval > new DateTime.now(
                ).millisecondsSinceEpoch) {
            return null;
        }
